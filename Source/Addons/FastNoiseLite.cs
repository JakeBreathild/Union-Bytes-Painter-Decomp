...
public class FastNoiseLite
{
		...
		default:
		{
			for (int i = xr - 1; i <= xr + 1; i++)
			{
				...
				for (int j = yr - 1; j <= yr + 1; j++)
				{
					...
					float num = (float)i - x + RandVecs2D[idx2] * cellularJitter;
					float vecY2 = (float)j - y + RandVecs2D[idx2 | 1] * cellularJitter;
		...
		case CellularDistanceFunction.Manhattan:
		{
			for (int k = xr - 1; k <= xr + 1; k++)
			{
				...
				for (int l = yr - 1; l <= yr + 1; l++)
				{
					...
					float f = (float)k - x + RandVecs2D[idx3] * cellularJitter;
					float vecY3 = (float)l - y + RandVecs2D[idx3 | 1] * cellularJitter;
					float newDistance3 = FastAbs(f) + FastAbs(vecY3);
			...
			for (int i = xr - 1; i <= xr + 1; i++)
			{
				...
				for (int j = yr - 1; j <= yr + 1; j++)
				{
					...
					for (int k = zr - 1; k <= zr + 1; k++)
					{
						...
						float num = (float)i - x + RandVecs3D[idx2] * cellularJitter;
						float vecY2 = (float)j - y + RandVecs3D[idx2 | 1] * cellularJitter;
						float vecZ2 = (float)k - z + RandVecs3D[idx2 | 2] * cellularJitter;
			...
			for (int l = xr - 1; l <= xr + 1; l++)
			{
				...
				for (int m = yr - 1; m <= yr + 1; m++)
				{
					...
					for (int n = zr - 1; n <= zr + 1; n++)
					{
						...
						float f = (float)l - x + RandVecs3D[idx3] * cellularJitter;
						float vecY3 = (float)m - y + RandVecs3D[idx3 | 1] * cellularJitter;
						float vecZ3 = (float)n - z + RandVecs3D[idx3 | 2] * cellularJitter;
						float newDistance3 = FastAbs(f) + FastAbs(vecY3) + FastAbs(vecZ3);
						distance1 = FastMax(FastMin(distance1, newDistance3), distance0);
						if (newDistance3 < distance0)
						{
							distance0 = newDistance3;
	...
	private float SinglePerlin(int seed, float x, float y)
	{
		...
		float a = Lerp(GradCoord(seed, x2, y2, xd0, yd0), GradCoord(seed, x3, y2, xd1, yd0), xs);
		float xf1 = Lerp(GradCoord(seed, x2, y3, xd0, yd1), GradCoord(seed, x3, y3, xd1, yd1), xs);
		return Lerp(a, xf1, ys) * 1.4247692f;
	}
	...
	private float SinglePerlin(int seed, float x, float y, float z)
	{
		...
		float a = Lerp(GradCoord(seed, x2, y2, z2, xd0, yd0, zd0), GradCoord(seed, x3, y2, z2, xd1, yd0, zd0), xs);
		...
		return Lerp(a2, yf1, zs) * 0.9649214f;
	}
	...
	private float SingleValue(int seed, float x, float y)
	{
		int x2 = FastFloor(x);
		int y2 = FastFloor(y);
		float xs = InterpHermite(x - (float)x2);
		float ys = InterpHermite(y - (float)y2);
		x2 *= 501125321;
		y2 *= 1136930381;
		int x3 = x2 + 501125321;
		int y3 = y2 + 1136930381;
		float a = Lerp(ValCoord(seed, x2, y2), ValCoord(seed, x3, y2), xs);
	...
	private float SingleValue(int seed, float x, float y, float z)
	{
		...
		float xs = InterpHermite(x - (float)x2);
		float ys = InterpHermite(y - (float)y2);
		float zs = InterpHermite(z - (float)z2);
		x2 *= 501125321;
		y2 *= 1136930381;
		z2 *= 1720413743;
		int x3 = x2 + 501125321;
		int y3 = y2 + 1136930381;
		int z3 = z2 + 1720413743;
		float a = Lerp(ValCoord(seed, x2, y2, z2), ValCoord(seed, x3, y2, z2), xs);
	...
	private void SingleDomainWarpBasicGrid(int seed, float warpAmp, float frequency, float x, float y, ref float xr, ref float yr)
	{
		...
		int x2 = FastFloor(num);
		int y2 = FastFloor(yf);
		float xs = InterpHermite(num - (float)x2);
		float ys = InterpHermite(yf - (float)y2);
		x2 *= 501125321;
		y2 *= 1136930381;
		int x3 = x2 + 501125321;
		int y3 = y2 + 1136930381;
	...
	private void SingleDomainWarpBasicGrid(int seed, float warpAmp, float frequency, float x, float y, float z, ref float xr, ref float yr, ref float zr)
	{
		...
		float xs = InterpHermite(num - (float)x2);
		float ys = InterpHermite(yf - (float)y2);
		float zs = InterpHermite(zf - (float)z2);
		x2 *= 501125321;
		y2 *= 1136930381;
		z2 *= 1720413743;
		int x3 = x2 + 501125321;
